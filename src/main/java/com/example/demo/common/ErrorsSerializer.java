...
@JsonComponent
public class ErrorsSerializer extends JsonSerializer<Errors> { // 이게 필요한 이유는 객체는 json 으로 Serialize 해서 리턴 할 수 있다.
    ...
    public void serialize(Errors errors, JsonGenerator gen, SerializerProvider serializers) throws IOException { // serialize 함수를 override 한다.
        ...
        errors.getFieldErrors().forEach( e -> { // Validator 클래스에서 rejectValue 라고 작성되어 있다면 이 부분이 사용된다.
            try {
        ...
        errors.getGlobalErrors().forEach(e -> { // Validator 클래스에서 reject 라고 작성되어 있다면 이 부분이 사용된다.
            try{
                gen.writeStartObject();
                gen.writeStringField("objectName", e.getObjectName());
                gen.writeStringField("code", e.getCode());
